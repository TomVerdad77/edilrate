  ...
  try {
    ...
    if (!id || !uuidRegex.test(id)) {
      return NextResponse.json(
        { error: "Feedback non valido." },
        { status: 400 }
      );
    }

    if (!allowedStatuses.includes(status as FeedbackStatus)) {
      return NextResponse.json(
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: "Impossibile aggiornare il feedback." },
        { status: 500 }
      );
    }
    ...
    console.error("Admin feedback status API error:", error);
